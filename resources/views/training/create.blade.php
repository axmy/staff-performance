<x-layouts.app>
    ...
    <div class="mb-6">
        <a href="{{ route('training.index') }}" class="text-indigo-600 hover:text-indigo-900">&larr; Back to Training List</a>
    </div>
            ...
            <form method="POST" action="{{ route('training.store') }}">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <x-form.input name="title" label="Training Title" :value="old('title')" :required="true" />
                    </div>
                    ...
                    <x-form.input name="trainer" label="Trainer" :value="old('trainer')" :required="true" />

                    <x-form.input name="location" label="Location" :value="old('location')" />

                    <x-form.date-input name="scheduled_date" label="Date" :value="old('scheduled_date')" :required="true" />

                    <x-form.date-input name="scheduled_time" label="Time" type="time" :value="old('scheduled_time')" :required="true" />

                    <x-form.input name="duration_minutes" label="Duration (minutes)" type="number" :value="old('duration_minutes')" :required="true" step="1" />

                    <x-form.select name="status" label="Status" :required="true">
                        <option value="upcoming" @if(old('status', 'upcoming') == 'upcoming') selected @endif>Upcoming</option>
                        ...
                        <div class="border border-gray-200 rounded-xl p-4 max-h-96 overflow-y-auto bg-gray-50/50">
                            ...
                            <label for="staff_{{ $member->id }}" class="flex items-center p-3 mb-2 last:mb-0 bg-white rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/50 cursor-pointer transition-colors duration-150">
                                <input type="checkbox" name="staff_ids[]" id="staff_{{ $member->id }}"
                                       value="{{ $member->id }}"
                                       @if(in_array($member->id, old('staff_ids', []))) checked @endif
