<x-layouts.app>
    <x-slot name="title">Attendance Import History</x-slot>
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            ...
                            <div class="min-w-0">
                                <div class="text-sm font-medium text-gray-900 truncate">{{ $import->original_file_name ?? $import->file_name }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">
                            {{ date('F Y', mktime(0, 0, 0, $import->month, 1, $import->year)) }}
                        </div>
                        <div class="text-xs text-gray-500">
                            {{ $import->year }}-{{ str_pad($import->month, 2, '0', STR_PAD_LEFT) }}
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">{{ $import->importer->name ?? 'N/A' }}</div>
                        <div class="text-xs text-gray-500">{{ $import->importer->email ?? '' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">{{ $import->created_at->format('d M Y') }}</div>
                        <div class="text-xs text-gray-500">{{ $import->created_at->format('H:i') }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                        <a href="{{ route('attendance.show', $import) }}"
                           class="inline-flex items-center font-medium text-indigo-600 hover:text-indigo-800 transition-colors mr-3">
                            Details
                        </a>
                        @if(auth()->user()->canManage())
                        <form method="POST" action="{{ route('attendance.destroy', $import) }}" class="inline"
            ...
            <div class="p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <dd class="mt-1 text-2xl font-bold text-gray-900">{{ $stats['successful'] }}</dd>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <dd class="mt-1 text-2xl font-bold text-gray-900">{{ $stats['partial'] }}</dd>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <dd class="mt-1 text-2xl font-bold text-gray-900">{{ $stats['failed'] }}</dd>
                    </div>
                </div>
            </div>
        </div>
