        ...
        <nav class="-mb-px flex space-x-1 overflow-x-auto" aria-label="Settings">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $tabs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tab): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route($tab['route'])); ?>"
                   class="<?php if(request()->routeIs($tab['route'] . '*')): ?> border-indigo-500 text-indigo-600 bg-indigo-50/50 <?php else: ?> border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 hover:bg-gray-50 <?php endif; ?> whitespace-nowrap flex items-center gap-2 border-b-2 py-3 px-4 text-sm font-medium rounded-t-lg transition-colors duration-150">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><?php echo $tab['icon']; ?></svg>
...
<?php /**PATH /Users/axmeen/Projects/staff-performance/resources/views/components/settings-nav.blade.php ENDPATH**/ ?>